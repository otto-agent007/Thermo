...
def test_backend_matches_exact_k30_and_reuses_deterministic_compilation(
    ...
    compiler_calls = 0
    compiled_identities: dict[str, tuple[str, tuple[float, ...]]] = {}
    lower_calls = 0
    compile_calls = 0
    ...
    def counted_compile_target(*args, **kwargs):
        ...
        compiled_identities[artifact.target_hash] = (
    ...
    # Fresh optimizer outputs need not match an archived runner's floating-point
    # endpoints. Deep validation above recomputes each identity from frozen values;
    # the record must preserve the actual compiler outputs exactly.
    assert {
        item.target_hash: (item.artifact_hash, item.optimization.parameters)
